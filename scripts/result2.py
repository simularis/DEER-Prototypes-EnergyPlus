...
Changelog
    ...
    * 2026-04-06 Changed backend to always write to SQLite, then export to CSV optionally.
...
def export_wide_table_from_sqlite(sqlfile: Path = 'simdata.sqlite',
                                  csvfile: Path = 'simdata.csv'):
    """Save a CSV report (simdata.csv) in wide table format for manual review.

    Caveats:
      (1) Data must already be stored in long table format in a database.
      (2) Does not support aggregation or wildcard queries. The query results
      are intended to be unique.

    The output file is the same as the one generated by gather_sim_data_to_csv,
    but this function reads from the SQLite database and writes to a CSV file in wide format.

    Inputs:
        sqlfile: Path
            Path to the SQLite database file containing sim_tabular table in long format.
            Default: 'simdata.sqlite'
        csvfile: Path
    ...
    conn = connect(sqlfile)
    try:
        # Read the long format data from sim_tabular
        df_long = pd.read_sql_query('SELECT * FROM sim_tabular', conn)
        
        if df_long.empty:
            print(f"Warning: No data found in sim_tabular table in {sqlfile}")
            conn.close()
            return
        
        # Create a combined column name from ColumnName, RowName, and Units
        df_long['combined_column'] = (
            df_long['ColumnName'].astype(str) + '/' + 
            df_long['RowName'].astype(str) + ' (' + 
            df_long['Units'].astype(str) + ')'
        )
        
        # Pivot the data from long to wide format
        # filename becomes the index, combined_column becomes columns, Value becomes values
        ...
        # Reset index so filename becomes a column instead of the index
        df_wide = df_wide.reset_index()
        
        # Write to CSV
        df_wide.to_csv(csvfile, index=False)
        print(f"Successfully exported wide table to {csvfile}")
        print(f"Shape: {df_wide.shape[0]} rows x {df_wide.shape[1]} columns")
        
    except Exception as e:
        print(f"Error reading from {sqlfile} or writing to {csvfile}: {e}")
        raise
    finally:
        conn.close()
...
def build_cli_parser(parser: argparse.ArgumentParser,
    ...
    parser.description = r"""Reads selected data entries from SQL outputs as well as DEER Peak period averages of hourly variables.

    Assumes that queries are not wildcarded (*) and that output values are unique for each
    query. If a query returns multiple rows, only the first row will be used. Please consider using
    result.py for wildcard queries (e.g. for normalizing units).

    Outputs:
        simdata.sqlite: A SQLite database file containing a table "sim_tabular" with
            one row for each individual query result. This file can be used for further
            analysis in Python or for manual inspection with another tool that can read SQLite databases.
        simdata.csv: Optional report file containing the same data as simdata.sqlite, but
            displayed in a wide table format for manual review. This file is optional and
            can be generated by using the -c or --csv flag.
    """
    ...
    parser.add_argument('-o','--output', type=Path, default='simdata.sqlite',
                        help=r'Output destination for SQLite file, e.g. simdata.sqlite',
                        **outputfile_kwargs)
    parser.add_argument('-v','--csvfile', type=Path, default='simdata.csv',
                        help=r'Output destination for CSV file, e.g. simdata.csv',
                        **outputfile_kwargs)
    parser.add_argument('-P', '--parallel', action='store_false', help='Disable parallel mode.')
    parser.add_argument('-c', '--csv', action='store_true', help='Create a CSV output file (off by default).')
...
def cli_main():
    ...
    # Start the main process of gathering data and writing to output file(s).
    gather_sim_data_to_sqlite_long(pargs.study, pargs.queryfile, pargs.output, pargs.parallel)
    if pargs.csv:
        export_wide_table_from_sqlite(pargs.output, pargs.csvfile)
